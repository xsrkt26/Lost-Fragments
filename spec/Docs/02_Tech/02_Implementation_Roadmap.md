...
- 饰品总表已对齐 56 个：50 个非道具饰品已加载并接入通用效果与一轮非道具差异精修；6 个道具联动饰品保留为未启用数据，奖励/商店不会产出。
...
6. P2 道具联动饰品保护：已完成。51-56 号饰品保留在总表中但未启用，奖励/商店池过滤，直接获得时使用 no-op 效果。
7. F1 道具系统基础版本：暂缓，用户已确认先不实现。
8. 用户视频反馈的 UI 问题：Hub/整理背包/局内切换时背包布面板覆盖和层级重叠，排在本轮策划 P0/P1 需求之后处理。

## 三、下一轮建议开发顺序

### 1. 修复 Hub/整理背包/局内 UI 模式边界

用户视频反馈显示整理背包模式缺少明确退出按钮，背包布面板在 Hub/局内复用时容易产生覆盖和层级重叠。下一轮优先把整理背包的入口、退出按钮、鼠标操作和布局约束稳定下来；如果继续复用 `main_game_ui.tscn` 不可靠，再拆出独立整理背包 scene。
