...
func test_black_water_bottle_transfer():
	...
	# 总 6 + 7 = 13.
	assert_eq(backpack.grid[Vector2i(2, 0)].current_pollution, 13)
...
func test_black_water_bottle_to_water_item():
	...
	backpack.place_item(sponge, Vector2i(2, 0))
	...
	# 海绵被撞：Multi=6. 海绵目前 placeholder 逻辑（由于我刚改了 tres 但没写逻辑，它现在应该也有自己的逻辑了）
	# 腐蚀海绵被撞：给路径之后所有物品 +1. 这里没有后续物品。
	assert_eq(backpack.grid[Vector2i(2, 0)].current_pollution, 5)
...
func test_corrosive_sponge_linear_spread():
	...
	backpack.place_item(sponge, Vector2i(2, 0)) # (2,0), (3,0)
	backpack.place_item(p1, Vector2i(2, 2))
	backpack.place_item(p2, Vector2i(2, 4))
	...
	# 逻辑：海绵向下方(DOWN)扫描。
	# p1(2,2) 和 p2(2,4) 都会被腐蚀 +1.
	# 此外，海绵会传导能量给 p1. p1 被撞自增 Multi=2 (+2).
	# p1 最终 = 1 + 2 = 3.
	# p2 只被腐蚀，没被撞（因为 p1 阻挡了传导流，且 p1 NONE 传导）。
	assert_eq(backpack.grid[Vector2i(2, 2)].current_pollution, 3)
	assert_eq(backpack.grid[Vector2i(2, 4)].current_pollution, 1)
...
func test_synergy_fluid_pollution_cascade():
	...
	backpack.place_item(bottle, Vector2i(0, 0))
	backpack.place_item(sponge, Vector2i(2, 0)) # Sponge is 2x1 (2,0),(3,0). Dir DOWN.
	backpack.place_item(paper, Vector2i(2, 2))
	...
	# 3. 海绵效果：被撞，向下腐蚀一列。纸团 (2,2) 获得 +1 污染。
	# 4. 海绵传导：海绵(2,0) 向下(DOWN) 击中纸团。
	#    此时纸团有 1 层。Multiplier=2. 纸团自增 +2.
	# 5. 纸团最终 = 1 + 2 = 3.
	assert_eq(backpack.grid[Vector2i(2, 0)].current_pollution, 4, "Sponge got pollution from bottle")
	assert_eq(backpack.grid[Vector2i(2, 2)].current_pollution, 3, "Paper got corroded and hit")
